...
// import { adminConfigurationsContainer } from "../config/cosmosdb.adminConfigurations.config"; // Adjust the import path as necessary
...
export const verifyAdminAuth = async (
  ...
  try {
    ...
    // Fetch the admin configuration document from the database
    try {
      //   const { resource: adminConfigDoc } = await adminConfigurationsContainer
      //     .item("AdminAccessControl", "AdminAccessControl")
      //     .read();
      //   // Check if the address is in the adminWallets list
      //   if (!adminConfigDoc.adminWallets.includes(address)) {
      //     return res.status(403).send("Access denied: not an admin");
      //   }
    } catch (dbError) {
      console.error("Database error when checking admin status:", dbError);
